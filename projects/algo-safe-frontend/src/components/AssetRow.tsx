...
export function AssetRow({ a }: { a: AssetHolding }) {
  return (
    <tr className="border-b border-outline-variant last:border-0">
      <td className="py-3"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-surface-container-high text-primary"><Icon name={icon[a.type]} className="text-lg" /></span>
        <div><div className="font-medium text-on-surface">{a.name}</div><div className="font-mono text-xs text-on-surface-variant">{a.symbol}{a.assetId ? ` · ${a.assetId}` : ''}{a.apy ? ` · ${a.apy}% APY` : ''}</div></div></div></td>
