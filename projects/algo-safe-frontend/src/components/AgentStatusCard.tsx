...
const pulse: Record<Agent['status'], string> = {
  active: 'bg-primary', paused: 'bg-on-surface-variant', pending_review: 'bg-warn',
}
  ...
  return (
    <Card>
      ...
      <div className="mt-4">
        ...
        <div className="mt-1 text-xs text-on-surface-variant">{dailyUsed.toLocaleString()} / {agent.dailyLimit.toLocaleString()} {agent.primaryAsset} · {agent.groupTier}</div>
      </div>
    </Card>
  )
}
