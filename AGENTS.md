...
Before writing any smart contract, understand what the AVM actually is — a stack machine with two types (`uint64`, `bytes`) and hard resource limits. LLMs default to TEAL/PyTeal and treat contract code like normal TypeScript. The `algorand-core` skill resets this mental model.
...
3. **Get examples**: Use `github_get_file_contents` from:
   ...
   - `algorandfoundation/puya-ts` (TypeScript)
...
3. **Run scripts**: `npx tsx scripts/call-contract.ts` (TS)
...
1. **Pick language**: TypeScript
2. **Load the skill**: `algorand-x402-typescript`
