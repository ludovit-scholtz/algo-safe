...
- **Cooldown enforcement**: `SignerGroup.cooldownRounds` (if nonzero) requires `Global.round >= group.lastExecutionRound + cooldownRounds` before a transaction-group proposal can execute; `lastExecutionRound` is updated on every execution. `cooldownRounds` is capped at `MAX_COOLDOWN_ROUNDS` (10,000,000 rounds, roughly a year at current block times) by `_validateAdminChange` on both `ADM_CREATE_GROUP` and `ADM_SET_POLICY` — an earlier version left this unbounded, which let a single oversized value (e.g. a rounds/seconds unit mix-up) trigger an AVM arithmetic-overflow panic in the `+` above and permanently freeze the group's execution until a follow-up policy fix
