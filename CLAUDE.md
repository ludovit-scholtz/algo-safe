...
| `smart_contracts/algo_safe_validator/contract.algo.ts` | **Validator library contract** — payload validation called via C2C (immutable, hash-pinned) |
| `smart_contracts/shared/types.ts` | Payload structs + TX_*/ACT_*/PRIV_*/GT_* constants shared by both contracts — never fork |
| `smart_contracts/algo_safe/validator-hash.generated.ts` | Pinned validator bytecode hash (auto-generated by `scripts/sync-validator-hash.ts` during `pnpm build`) |
| `smart_contracts/algo_safe/contract.e2e.spec.ts` | E2E tests |
| `src/validator.ts` | Validator registry (`VALIDATOR_DEPLOYMENTS`), hash-verifying `resolveValidatorAppId`, `deployValidator` |
...
- **Box pruning**: `pruneProposal(proposalId, ensureBudgetValue)` deletes a terminal (`STATUS_EXECUTED`/`STATUS_CANCELLED`) proposal's box and payload boxes to reclaim MBR. `STATUS_EXECUTED` proposals are prunable immediately — a past-expiry gate would add no safety since the ledger already retains the execution record — while `STATUS_CANCELLED` proposals still require `Global.round > expiryRound`, kept as a review-retention window for a cancelled-but-contested proposal (v3.2.0; 2026-07-16 audit M-01 — previously both statuses required past-expiry, which meant any proposal created with a far-future `expiryRound` could never be pruned). `getActivePrivGroupCount()` is the read-only getter for the lockout counter above
...
- **Membership-epoch invalidation**: `SignerGroup.membershipEpoch` increments every time a member is removed (`_adminRemoveMember`); each proposal snapshots `epochAtCreation` at creation time, and both `approveProposal` and execution assert the group's live epoch still matches — removing a member invalidates every pending proposal's already-recorded approvals — both `approveProposal` and execution assert the group's *live* epoch matches, so a stranded proposal can only be cancelled and re-created, never re-approved (2026-07-16 audit I-02: prior wording said "re-approved from scratch", which is not possible), closing the window where a since-removed (e.g. compromised) signer's stale approval still counted toward a threshold
- **Custodian groups & asset guards**: `SignerGroup.groupType` discriminates `GT_STANDARD=0` (normal admin-governed group) from `GT_CUSTODIAN=1` (protocol custodian, zero admin privileges, bounded by asset guards). Key properties: (1) `ADM_CREATE_CUSTODIAN=11` creates a custodian group — `_createGroup` forces `adminPrivileges=0` regardless of the field in `AdminChange`; (2) asset guards are stored in a BoxMap `assetGuards` (prefix `'ag'`, key `{custodianGroupId, assetId}`, value `{createdRound, lockedAmount}`) — `assetId=0` means ALGO; (3) `ADM_SET_GUARD=13` / `ADM_REMOVE_GUARD=14` are admin-only, require `PRIV_GROUP`, create/update/delete guards and maintain `group.guardCount`; (4) in `_executeTransactionGroup` custodian groups skip daily/monthly limit checks entirely and instead call `_deductFromGuard(custodianGroupId, assetId, amount)` for each TX_PAYMENT (assetId=0) and TX_ASSET (assetId=tx.xferAsset) — `_deductFromGuard` asserts a guard box exists for `(custodianGroupId, assetId)` **even for a 0-amount transfer** (v3.2.0; 2026-07-16 audit L-01 — previously the zero-amount case early-returned before the existence check, letting a custodian opt the safe into arbitrary ASAs via a 0-amount transfer with no guard configured; admins must now set a guard, even `lockedAmount: 0`, before a custodian may touch an asset at all) and reverts atomically if the inner txn group fails; (5) `ADM_DISSOLVE_CUSTODIAN=12` must be proposed by the custodian group itself (admin groups cannot — checked in `proposeAdminChange`), blocked at execution unless `group.guardCount === 0` **and** `group.memberCount === 1`, with the dissolve change's `memberAddr` naming that last member — the member box is deleted with the group box so no MBR is orphaned (2026-07-12 audit M-01; remove extra members first via `ADM_REMOVE_MEMBER`, which reclaims their boxes); a dissolved group's terminal, past-expiry proposals stay prunable — `pruneProposal` skips the membership check once the group box no longer exists (audit M-02); (6) `ADM_SET_PRIVILEGES` on a custodian group reverts at execution time; (7) custodian groups cannot rekey — the validator asserts `groupType !== GT_CUSTODIAN`. Read guards with `readAssetGuard` (`src/on-chain.ts`)
...
- **v3.0.0 externalised payload validation and removed all read-only ABI getters (breaking)**. Motivation: the approval program had hit 8 187/8 192 bytes; v3.0.0 lands at ~6 900 (≈1.3 KB reclaimed). Three changes:
  - **Validator library contract**: payload validation for TX_PAYMENT/TX_ASSET/TX_KEYREG/TX_ACFG/TX_REKEY moved to `AlgoSafeTxnValidator` (`smart_contracts/algo_safe_validator/`, ~420 bytes, stateless, rejects update AND delete → immutable). The safe calls its `validateTxn(txType, data, allowedActions, adminPrivileges, groupType) → (assetId, amount, hasClose, sender)` via inner app call per payload entry in `_executeTransactionGroup` pass 1 (each C2C call also *adds* 700 pooled opcode budget). **TX_APP stays validated in the safe** (`_validateApp`) — its appArgs can total 2 048 bytes and cannot fit through the inner-call arg limit. `createApplication(name, validatorAppId)` gained a second arg: the safe asserts `sha256(AppParams.appApprovalProgram(validatorAppId)) == VALIDATOR_APPROVAL_SHA256_HEX` (from `validator-hash.generated.ts`, maintained by `scripts/sync-validator-hash.ts` inside `pnpm build`) and stores it in GlobalState `vapp`; since the pinned bytecode can never be updated or deleted, the one-time check holds forever. Executions therefore need the validator app in resources (auto-populated by `populateAppCallResources`; tests that pass explicit resources must add `appReferences: [validatorAppId]`) and cost +1 min-fee per non-app payload entry. Off-chain: `src/validator.ts` has `VALIDATOR_DEPLOYMENTS` (per-genesis-hash registry — fill after first TestNet/MainNet deploy), `resolveValidatorAppId` (verifies bytecode hash before returning), `deployValidator`. `deployClonedSafe` gained optional `validatorAppId`; frontend create/upgrade flows resolve-or-deploy.
  - **All 12 read-only getters removed** (`getConfig`, `getSignerGroup`, `getProposal`, `getTransactionGroup`, `getMember`, `isMember`, `hasApproved`, `getActivePrivGroupCount`, `isRekeyedAddress`, `getRekeyedAddress`, `hasAssetGuard`, `getAssetGuard`). Replacements in `src/on-chain.ts`: `readSafeConfig`, `readSignerGroup`, `readMember`, `readIsMember`, `readProposal`, `readTransactionGroup`, `readHasApproved`, `readRekeyedAddress`, `readAssetGuard` — they use the typed client's `state.box.*`/`state.global.*` accessors, so they work against **every** deployed contract version (frontend still calls the legacy getters on pre-v3 safes via its `isLatest` branch, but the readers are correct there too except `readTransactionGroup`, whose `proposalId*7+slot` box keying is latest-only). Missing box ⇒ `undefined` (the old getters reverted).
  - **Shared types module**: the payload structs and TX_*/ACT_*/PRIV_*/GT_* constants moved to `smart_contracts/shared/types.ts`, imported by both contracts. `bootstrap()` was also rewritten on top of `bootstrapGroup`'s internals (`_seedGroup`) — same ABI, and note `GroupCreated` is now emitted *after* the `MemberAdded` events instead of before.
  - **2026-07-12 audit remediations** (same release): `ADM_DISSOLVE_CUSTODIAN` now requires `memberCount === 1` and the change's `memberAddr` to name that last member (its box is deleted with the group — M-01); `pruneProposal` skips the membership check when the group box no longer exists (M-02); `ADM_CHANGE_THRESHOLD` asserts `threshold >= 1` and `_createGroup` asserts `threshold === 1` (L-01); `_adminAddMember` rejects the zero address (L-02). Callers building dissolve proposals **must now set `memberAddr`**.
- **v3.1.0 hardened admin-change input validation (non-breaking; 2026-07-12 Fable-5 audit remediations)**: (M-01) **custodian `allowedActions` restricted to `ACT_PAY|ACT_AXFER`** — asserted in `_seedGroup`, `_createGroup` (`ADM_CREATE_CUSTODIAN`) and `ADM_SET_POLICY` on custodian targets, because asset guards only meter pay/axfer value; a custodian can no longer hold `ACT_APPL`/`ACT_ACFG`/`ACT_KEYREG`/`ACT_REKEY`; (L-01) `_createGroup` rejects a zero-address initial member; (L-02) `ADM_ADD_REKEYED_ADDR` rejects the zero address and `buildMigrationRekeyPayload` throws on one (a zero registry entry would premature-self-rekey the safe mid-migration group); (I-01) `_createGroup` bounds `allowedActions <= ACT_ALL` and `adminPrivileges <= PRIV_ALL`. Approval program: 7,043 bytes.
- **v3.2.0 closed two MBR/containment gaps (non-breaking; 2026-07-16 Fable-5 audit remediations)**: (M-01) `pruneProposal` no longer requires past-expiry for `STATUS_EXECUTED` proposals (only `STATUS_CANCELLED` still does) — a terminal, executed proposal created with a far-future `expiryRound` (e.g. the library's `FAR_EXPIRY` constant) is now prunable immediately instead of permanently locking its box MBR; (L-01) `_deductFromGuard` now asserts a guard exists for `(custodianGroupId, assetId)` **before** its zero-amount early return, so a custodian can no longer opt the safe into an unguarded ASA via a 0-amount transfer. Off-chain (`algo-safe` npm package, same release): `algosdkTxnsToSafeTxnGroup` now throws on a keyreg with `nonParticipation: true` instead of silently downgrading it to a plain go-offline (I-01); `fetchSafeCloneConfig` cross-checks each group's enumerated member-box count against its on-chain `memberCount` and masks a cloned custodian group's `allowedActions` to `ACT_PAY|ACT_AXFER` with a logged warning (I-03); the box-enumeration helpers in `on-chain.ts`/`migration.ts` now throw a clear error when algod's non-paginated boxes endpoint returns a result that exactly fills the request cap, instead of silently risking a truncated read (L-02).
...
- **The build enforces a program-size gate** (`scripts/check-program-size.ts`, audit C-01): it fails when the compiled approval program exceeds **7,800 bytes** (AVM hard ceiling 8,192). If a change trips the gate, reduce size — don't raise the threshold without review. Current size after the v3.0.0 reduction pass: ~6,970 bytes.
