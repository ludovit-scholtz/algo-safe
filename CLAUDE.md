...
- **Custodian groups & asset guards**: `SignerGroup.groupType` discriminates `GT_STANDARD=0` (normal admin-governed group) from `GT_CUSTODIAN=1` (protocol custodian, zero admin privileges, bounded by asset guards). Key properties: (1) `ADM_CREATE_CUSTODIAN=11` creates a custodian group — `_createGroup` forces `adminPrivileges=0` regardless of the field in `AdminChange`; (2) asset guards are stored in a BoxMap `assetGuards` (prefix `'ag'`, key `{custodianGroupId, assetId}`, value `{createdRound, lockedAmount}`) — `assetId=0` means ALGO; (3) `ADM_SET_GUARD=13` / `ADM_REMOVE_GUARD=14` are admin-only, require `PRIV_GROUP`, create/update/delete guards and maintain `group.guardCount`; (4) in `_executeTransactionGroup` custodian groups skip daily/monthly limit checks entirely and instead call `_deductFromGuard(custodianGroupId, assetId, amount)` for each TX_PAYMENT (assetId=0) and TX_ASSET (assetId=tx.xferAsset) — `_deductFromGuard` reverts atomically if the inner txn group fails; (5) `ADM_DISSOLVE_CUSTODIAN=12` must be proposed by the custodian group itself (admin groups cannot — checked in `proposeAdminChange`), blocked at execution unless `group.guardCount === 0`; (6) `ADM_SET_PRIVILEGES` on a custodian group reverts at execution time; (7) custodian groups cannot rekey — `_validateRekey` asserts `group.groupType !== GT_CUSTODIAN`. Read guards with `hasAssetGuard` / `getAssetGuard`
- **Validation timing**: `proposeAdminChange` does minimal upfront checks (proposer is active member, group is active, dissolve-custodian routing) — most correctness checks (e.g. target-group type, guard existence) are deferred to execution in `_applyAdminChange`. This trades wasted box storage on invalid proposals for a ~700-byte reduction in compiled binary size
...
- **v2.0.0 introduced Custodian Groups (breaking `AdminChange` type)**: `AdminChange` lost `guardNote` and `secondaryGroupId` (the PT_LOCK dual-key mechanism was dropped entirely). `AdminChange` gained `guardAmount` (used by `ADM_SET_GUARD=13` as the guard `lockedAmount`; `limitAssetId` doubles as the guard's `assetId`). New group type `GT_CUSTODIAN=1` added to `SignerGroup.groupType`. New constants: `ADM_CREATE_CUSTODIAN=11`, `ADM_DISSOLVE_CUSTODIAN=12`, `ADM_SET_GUARD=13`, `ADM_REMOVE_GUARD=14`, `GT_STANDARD=0`, `GT_CUSTODIAN=1`. Removed: `PT_LOCK`, `ADM_SET_PAUSED` as an explicit constant (the pause handler is an `else` branch; `ADM_SET_PAUSED=10` stays in `src/constants.ts` for external callers but is not declared in the contract). `proposeAdminChange` no longer calls `_validateAdminChange` at proposal time — correctness validation deferred to execution. `src/admin.ts`'s `createAdminChange()` no longer has a `guardNote` default.
