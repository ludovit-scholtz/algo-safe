...
- name of the TLS secret already present in the `algo-safe` namespace, for example `algo-safe-frontend-tls`
...
4. A TLS secret in the target namespace matching `FRONTEND_TLS_SECRET_NAME`, or an external controller such as cert-manager that creates it.
...
`ingress.yaml` uses `FRONTEND_HOST_PLACEHOLDER` and `FRONTEND_TLS_SECRET_PLACEHOLDER`. The CD workflow replaces those values from GitHub repository variables.
...
6. Push to `main` and confirm the `Release` workflow completes.